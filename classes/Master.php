...
Class Master extends DBConnection {
	...
	function save_offense(){
		...
		if(empty($id)){
			...
			$color = '#' .  $rand;
			$select_id = "SELECT MAX(id) FROM `offenses`";
			$result = $this->conn->query($select_id);
			$max_id = intval($result->fetch_row()[0]);

			$color_query = "UPDATE`offenses` SET color = '" . $color . "' WHERE id = {$max_id};";
			$this->conn->query($color_query);
		}else{
	...
	function save_driver(){
		...
		// $chk = $this->conn->query("SELECT * FROM `drivers_list` where  age = '{$age}' ".($id>0? " and id!= '{$id}' " : ""))->num_rows;
		// $this->capture_err();
		// if($chk > 0){
		// 	$resp['status'] = 'failed';
		...
		// }
		if(empty($id))
			$sql1 = "INSERT INTO `drivers_list` set `name` = '{$name}', age = '{$age}' ";
		else
			$sql1 = "UPDATE `drivers_list` set `name` = '{$name}', age = '{$age}' where id = '{$id}' ";
